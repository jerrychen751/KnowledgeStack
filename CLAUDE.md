...
| The session cookie | `apps/api/src/auth/auth.module.ts:23` and `apps/api/src/auth/session.guard.ts:28` | `SessionGuard` runs under `APP_GUARD` on every route of every module. A route without `@Public()` never runs for a signed-out browser, so `request.session` carries a live session. |
